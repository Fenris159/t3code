...
- Runtime-adjusted roles must be ordinary custom properties shared by the Tailwind bridge, global CSS, and imperative style strings. Audit literal `var(--foreground)`, `var(--border)`, and related role reads so headings, markdown chrome, menus, and utilities do not split into adjusted and unadjusted colors.
